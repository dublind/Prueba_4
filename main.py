...
def cargar_horarios(colaboradores):
    ...
    if conn:
        try:
            ...
            for fila in cursor.fetchall():
                nombre, mes, dia, hora, rut_cliente, nombre_cliente = fila
...
def cargar_pagos():
    ...
    if conn:
        try:
            ...
            for fila in cursor.fetchall():
                rut, nombre, boleta, tipo_pago = fila
                pagos[boleta] = {  # Usar boleta como clave única
                    "rut": rut,
                    "nombre": nombre,
                    "tipo_pago": tipo_pago
                }
...
def cargar_atencion():
    atencion_clientes = {}  # Cambio a diccionario usando correo como clave
    conn = conectar()
    if conn:
        try:
            cursor = conn.cursor()
...
def imprimir_tabla_horarios(colaborador):
    conn = conectar()
    if conn:
        try:
            cursor = conn.cursor()
            ...
            print(f"Error al imprimir tabla de horarios: {e}")
        finally:
            conn.close()
    ...
    if opcion == "1":
                        ...
                        if editar == "si":
                            ...
                            try:
                                ...
                                if 1 <= num <= len(claves_horas):
                                    ...
                                    if not nuevo_mes.isdigit() or not (1 <= int(nuevo_mes) <= 12):
                                        print("Mes inválido. Debe ser entre 1 y 12.")
                                        continue
                                    
                                    print("Ingrese el nuevo día:")
                                    nuevo_dia = input().strip()
                                    if not nuevo_dia.isdigit() or not (1 <= int(nuevo_dia) <= 31):
                                        print("Día inválido.")
                                        continue
                                        ...
                                        if nueva_clave in horario:
                                            ...
                                            if eliminar_horario(nombre_colab, clave_a_editar[0], clave_a_editar[1], hora_a_eliminar):
                                                ...
                                                if insertar_horario(nombre_colab, nuevo_mes, nuevo_dia, hora_txt, datos_usuario['rut_cliente'], datos_usuario['nombre']):
                                                    ...
                                                    print("Error al insertar la nueva hora. Intentando restaurar...")
                                                    # Si falla la inserción, intentar restaurar
                                                    insertar_horario(nombre_colab, clave_a_editar[0], clave_a_editar[1], hora_a_eliminar, datos_usuario['rut_cliente'], datos_usuario['nombre'])
                                            else:
                                                print("Error al eliminar la hora actual de la base de datos.")
                                else:
                        ...
                        if insertar_horario(bar, mes, dia, hora_txt, rut, usuarios[rut]['nombre']):
                            ...
                            if tipo_pago == "1":
                                ...
                                if insertar_pago(rut, usuarios[rut]['nombre'], boleta, "Tarjeta"):
                                    pagos[boleta] = {  # Usar boleta como clave
                                        "rut": rut,
                                        "nombre": usuarios[rut]['nombre'],
                                        "tipo_pago": "Tarjeta"
                                    }
        ...
        while seguir:
            ...
            if insertar_atencion(correo, nombre):
                atencion_clientes[correo] = {  # Usar correo como clave
                    "nombre": nombre
                }
...
#--------------
print("horario de Tomas: ")
print(imprimir_tabla_horarios("Tomas"))
print("horario de Agustin: ")
print(imprimir_tabla_horarios("Agustin"))
print("Usuarios:")
print(usuarios)
print("Colaboradores:")
print(colaboradores)
print("Horarios:")
print(cargar_horarios(cargar_colaboradores()))
print(cargar_pagos())
print("clientes que hicieron consultas: ")
print(atencion_clientes)
